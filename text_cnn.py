...
class TextCNN(object):
    ...
    def __init__(self,sequence_length,num_classes,dim,filter_sizes,num_filters,l2_reg_lambda=0.0):
        self.input_x = tf.placeholder(tf.float32,[sequence_length,dim],name="input_x");
        ...
        with tf.name_scope("embedding"):
            self.embedding_chars_expanded1 = tf.expand_dims(self.input_x,-1);
            self.embedding_chars_expanded=tf.expand_dims(self.embedding_chars_expanded1,0);
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s"%filter_size):
                ksize=[filter_size,dim,1,num_filters];
